...
let cameraControlsInterval: any;
let camera: any;
let zpos = 0;
let fov = 90;
let startPosition: { x: number; y: number; z: number };
let startCamPosition: { x: number; y: number; z: number };
...
export function setZPos(value: number) {
    let posZ = value;
...
export function setXPos(value: number) {
    let posX = value;

    native.setCamCoord(camera, startCamPosition.x - posX, startCamPosition.y, startCamPosition.z);
    native.pointCamAtCoord(camera, startPosition.x - posX, startPosition.y, startPosition.z);
    native.setCamActive(camera, true);
    native.renderScriptCams(true, false, 0, true, false, 0);
}